...
/** If a field is annotated with Inject and the object is constructed with context manager, the field value
...
public @interface Inject {
	/** Used to resolve conflicts if multiple components share an interface or abstract class - selected class
	 * will be the chosen implementation. If there is only one object of the variable's type in the context,
	...
	/** If true, a new instance of the variable's class will be injected rather than the one stored in context
	 * (if any is actually present). This instance will be unique to the object that has it injected, but it
	 * is still partially managed by the context (initiated, disposed, destroyed, but cannot be injected). */
