...
public class ContextContainer implements Disposable {
    ...
    private final ObjectMap<Class<? extends Annotation>, ObjectSet<ComponentFieldAnnotationProcessor>> fieldAnnotationProcessors = LazyObjectMap
            .newMapOfSets();
    private final ObjectMap<Class<? extends Annotation>, ObjectSet<ComponentMethodAnnotationProcessor>> methodAnnotationProcessors = LazyObjectMap
            .newMapOfSets();
    private final ObjectMap<Class<? extends Annotation>, ObjectSet<ComponentTypeAnnotationProcessor>> typeAnnotationProcessors = LazyObjectMap
            .newMapOfSets();
    private final ObjectMap<Class<? extends Annotation>, ObjectSet<ComponentMetaAnnotationProcessor>> metaAnnotationProcessors = LazyObjectMap
            .newMapOfSets();

    private EventProcessor eventProcessor;
    private MessageProcessor messageProcessor;

    /** Creates a new context container with the default annotations processors. To scan for actual context components,
     * {@link #registerComponents(Class, ClassScanner)} method has to be called. */
    public ContextContainer() {
        registerDefaultAnnotationProcessors();
        registerDefaultComponents();
    }

    /** @param scanningRootClass class in the root folder that determines which passed component classes are accepted.
    ...
    /** @param scanningRootClass class in the root folder that determines where are the annotated classes taken from.
    ...
    /** @param processor will be registered as meta annotation processor. If another processor is registered for the
     *            same annotation class, both processors will be honored. Meta annotations are given to classes that are
     *            also annotation processors. Meta components are processed before the others, allowing to expand
    ...
    public void unregisterMetaAnnotationProcessors(final Class<? extends Annotation> forAnnotation) {
        final ObjectSet<ComponentMetaAnnotationProcessor> processors = metaAnnotationProcessors.get(forAnnotation);
    ...
    /** @param processor will be registered as method annotation processor. If another processor is registered for the
     *            same annotation class, both processors will be honored. Upon component initiation, methods are
     *            processed after fields. */
    ...
    public void unregisterMethodAnnotationProcessors(final Class<? extends Annotation> forAnnotation) {
        final ObjectSet<ComponentMethodAnnotationProcessor> processors = methodAnnotationProcessors.get(forAnnotation);
        for (final ComponentAnnotationProcessor processor : processors) {
            unregisterProcessor(processor);
        }
        processors.clear();
    ...
    /** @param processor will be registered as field annotation processor. If another processor is registered for the
     *            same annotation class, both processors will be honored. Upon component initiation, fields are
     *            processed first. */
    ...
    public void unregisterFieldAnnotationProcessors(final Class<? extends Annotation> forAnnotation) {
        final ObjectSet<ComponentFieldAnnotationProcessor> processors = fieldAnnotationProcessors.get(forAnnotation);
        for (final ComponentAnnotationProcessor processor : processors) {
            unregisterProcessor(processor);
        }
        processors.clear();
    ...
    /** @param processor will be registered as type annotation processor. If another processor is registered for the
     *            same annotation class, both processors will be honored. Type annotations are given to classes - if a
     *            class contains one of processed type annotations, it is considered a component and it will be scanned
     *            for. */
    ...
    public void unregisterTypeAnnotationProcessors(final Class<? extends Annotation> forAnnotation) {
        final ObjectSet<ComponentTypeAnnotationProcessor> processors = typeAnnotationProcessors.get(forAnnotation);
        for (final ComponentAnnotationProcessor processor : processors) {
            unregisterProcessor(processor);
        }
        processors.clear();
     ...
     *
     * @param scanningRootClass class in the root folder that determines where are the annotated classes taken from.
     *            Root package is referenced by one of its classes instead of name to allow easy structure refactors.
    ...
    public void registerComponents(final Class<?> scanningRootClass, final ClassScanner classScanner) {
        ...
        registerMetaComponents(
                classScanner.findClassesAnnotatedWith(scanningRootClass, getComponentMetaAnnotationClasses()));
        // Regular components processing:
        registerScannedContextComponents(
                classScanner.findClassesAnnotatedWith(scanningRootClass, getComponentTypeAnnotationClasses()));
    ...
    /** For internal use of type processors only. Use {@link #putInContext(Class)} to manually add a context component.
     *
    ...
    private void registerDefaultComponents() {
        ...
        addComponentToContextClassHierarchyRecursively(new ContextComponent(getClass(), this, false, true, true));
    }
    ...
    private void registerMetaComponents(
        ...
        for (final Entry<Class<? extends Annotation>, ObjectSet<Class<?>>> classesToProcess : metaComponentClasses) {
            final ObjectSet<ComponentMetaAnnotationProcessor> metaComponentProcessors = metaAnnotationProcessors
                    .get(classesToProcess.key);
    ...
    /** For internal use and custom annotation processors. Allows to extract a wrapped context component without
     * triggering its full initiation.
     *
    ...
    /** Default method for manual component extraction from the context. As a rule of thumb, if you can extract a
     * component from the context through a injected field or method parameter, you should do that instead of calling
     * this method.
     *
     * @param componentClass class of the desired component.
    ...
    /** Specialized retrieval method that allows to return all components of the given type stored in the context.
     *
    ...
    /** This method fully initiates an object of the given class, but it does not register it in the main context
     * container. The instance will still be managed (as in Dispose and Destroy annotations will be honored on context
     * destruction), but it will not be accessible by {@link #getFromContext(Class)} or
     * {@link #extractFromContext(Class)}
     *
     * @param componentClass class of the component to initiate. This class has to be GWT-reflected in order to work
     *            properly.
    ...
    public ContextComponent constructRequestedComponent(final Class<?> componentClass) {
        ...
        for (final ObjectSet<ComponentTypeAnnotationProcessor> typeProcessors : typeAnnotationProcessors.values()) {
            for (final ComponentTypeAnnotationProcessor processor : typeProcessors) {
                ...
                if (ClassReflection.isAnnotationPresent(componentClass, processor.getProcessedAnnotationClass())) {
    ...
    /** @param componentClass an instance of this class will be fully initiated and mapped into the context for all of
     *            its super class and interfaces. This class has to be GWT-reflected in order to work properly. */
    ...
    /** @param componentClass class that points to a single component. This component will be destroyed and removed from
     *            context. */
    ...
    /** @param baseComponentClass components of this class - or any of this class' children - will be destroyed and
    ...
    /** Mostly for internal use and custom processors. When component is lazy and not initialized, it can be requested
     * to wake up by the processor - context initiates such components after regular fields and methods processing.
     *
    ...
    private void postDestructionEventFor(final Iterable<ContextComponent> componentsToDestroy) {
        final ComponentDestructionEvent destructionEvent = new ComponentDestructionEvent(
                GdxArrays.newArray(componentsToDestroy));
    ...
    private void clearUtilityComponentCollections() {
        GdxSets.clearAll(managedComponents, componentsToRegister, componentsToDestroy, requestedLazyComponents);
    }
