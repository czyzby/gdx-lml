    ...
    public boolean processEvent(final Object event) {
        try {
            final Object result = Reflection.invokeMethod(listenerMethod, listenerComponent,
                    prepareMethodParameters(event));
            return result instanceof Boolean ? ((Boolean) result).booleanValue() || removeAfterInvocation
                    : removeAfterInvocation;
        } catch (final Throwable exception) {
            if (strict) {
                throw new AutumnRuntimeException("Unable to execute event: " + event + " for method: " + listenerMethod
                        + " of component: " + listenerComponent + ".", exception);
            }
        }
