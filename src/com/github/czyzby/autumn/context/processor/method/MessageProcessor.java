...
/** OnMessage annotation processor and one of core, default components. Some processors use this one for specific
 * actions invocations. Do NOT remove from context, unless you want to replace all other default processors.
 *
 * @author MJ */
public class MessageProcessor extends ComponentMethodAnnotationProcessor {
     ...
     *
     * @param listener will act as if it received a message. If processing action returns true, it will be removed. */
    public void notifyListener(final ComponentMessageListener listener) {
    ...
    public void removeUnusedListeners() {
        while (GdxArrays.sizeOf(listenersToRemove) > 0) {
            ...
            for (final Object listener : listeners) {
                ...
                messageListeners.get(((ComponentMessageListener) listener).getMessageContent())
                        .remove((ComponentMessageListener) listener);
                removedListeners++;
            }
