...
public class DestroyAnnotationProcessor extends ComponentMethodAnnotationProcessor implements ComponentEventListener {
    private final ObjectMap<ContextComponent, Array<ComponentMethodInvocation>> scheduledInvocations = LazyObjectMap
            .newMapOfArrays();
    ...
    public void processMethod(final ContextContainer context, final ContextComponent component, final Method method) {
        ...
        scheduledInvocations.get(component)
                .add(new ComponentMethodInvocation(destructionData.priority(), method, context, component));
    }
    ...
    public boolean processEvent(final Object event) {
        for (final ContextComponent componentToDestroy : ((ComponentDestructionEvent) event).getComponentsToDestroy()) {
