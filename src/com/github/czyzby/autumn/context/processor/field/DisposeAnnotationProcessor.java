...
public class DisposeAnnotationProcessor extends ComponentFieldAnnotationProcessor implements
	...
	private final ObjectMap<ContextComponent, Array<ReflectedField>> fieldsToDispose = LazyObjectMap
			.newMapOfArrays();
	...
	public boolean processEvent(final Object event) {
		...
		for (final ContextComponent componentToDestroy : destructionEvent.getComponentsToDestroy()) {
			for (final ReflectedField field : fieldsToDispose.get(componentToDestroy)) {
	...
	private void disposeOf(final ReflectedField field, final Object component) {
		try {
			final Object fieldValue = field.get(component);
