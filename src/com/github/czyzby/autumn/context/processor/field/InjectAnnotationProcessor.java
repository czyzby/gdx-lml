...
public class InjectAnnotationProcessor extends ComponentFieldAnnotationProcessor {
    ...
    private static void injectLazyVariable(final ContextContainer context, final ContextComponent component,
        ...
        if (injectionData.concurrentLazy()) {
            lazyComponent = Lazy.concurrentProvidedBy(
                    getContextComponentProvider(context, injectionData.lazy(), injectionData.newInstance()));
        } else {
            lazyComponent = Lazy.providedBy(
                    getContextComponentProvider(context, injectionData.lazy(), injectionData.newInstance()));
        }
        try {
    ...
    private static void injectRegularVariable(final ContextContainer context, final ContextComponent component,
            final Field field, final Inject injectionData) {
        try {
            ...
            throw new AutumnRuntimeException("Unable to inject variable for component: " + component + ".", exception);
        }
    }
    ...
    private static void injectNewInstance(final ContextContainer context, final ContextComponent component,
        ...
        final Class<?> componentClass = ComponentAnnotations.isClassSet(injectionData.value()) ? injectionData.value()
                : field.getType();
    ...
    private static void injectExtractedFromContext(final ContextContainer context, final ContextComponent component,
            final Field field, final Inject injectionData) throws ReflectionException {
