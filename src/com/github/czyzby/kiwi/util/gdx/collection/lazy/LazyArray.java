 ...
 * the value connected with the given index is null, a new object is created, put into the array and returned.
 * This is especially useful for arrays of other collections, that are often created in a very similar way and
 * without the need of varying constructor parameters.
 *
 * @author Nathan Sweet
 * @author MJ */
public class LazyArray<Type> extends Array<Type> {
	...
	/** @param ordered If false, methods that remove elements may change the order of other elements in the
	 *            array, which avoids a memory copy.
	 ...
	 * @param capacity Any elements added beyond this will cause the backing array to be grown.
	 * @param provider creates new object on get(index) calls if the value on the selected index is null. */
	public LazyArray(final ObjectProvider<? extends Type> provider, final boolean ordered,
			final int capacity, final Class<Type> arrayType) {
	 ...
	 * type of backing array and will be ordered if the specified array is ordered. The capacity is set to the
	...
	/** Creates a new ordered array containing the elements in the specified array. The new array will have the
	 * same type of backing array. The capacity is set to the number of elements, so any subsequent elements
	...
	/** Creates a new array containing the elements in the specified array. The new array will have the same
	 * type of backing array. The capacity is set to the number of elements, so any subsequent elements added
	 ...
	 * @param provider creates new object on get(index) calls if the value on the selected index is null. */
	public LazyArray(final ObjectProvider<? extends Type> provider, final boolean ordered,
			final Type[] array, final int start, final int count) {
	...
	public static <Type> LazyArray<Type> of(final ObjectProvider<? extends Type> provider,
			final Type... values) {
	...
	public static <Type> LazyArray<Type> with(final ObjectProvider<? extends Type> provider,
			final Class<Type> forClass, final Iterable<? extends Type> values) {
