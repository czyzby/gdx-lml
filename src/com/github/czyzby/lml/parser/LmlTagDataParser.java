...
public interface LmlTagDataParser<Widget extends Actor> {
    ...
    /** @param parser will be added to the parsed attributes list. Default attributes can be also set globally by
     *            accessing static methods of tag parsers. */
    void registerAttributeParser(LmlTagAttributeParser parser);

    /** @param attributeName parser connected with the attribute name will be removed, if exists. Default attributes can
     *            be also unset globally by accessing static methods of tag parsers. */
