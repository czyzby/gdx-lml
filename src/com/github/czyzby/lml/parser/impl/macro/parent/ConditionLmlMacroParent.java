...
public class ConditionLmlMacroParent extends AbstractConditionalLmlMacroParent {
    public ConditionLmlMacroParent(final LmlMacroData lmlMacroData, final LmlParent<?> parent, final LmlParser parser) {
    ...
    protected boolean checkCondition(final LmlParser parser) {
        ...
        if (operator == null) {
            if (unparsedOperator.charAt(0) == ACTION_OPERATOR) {
                return (Boolean) parser.findAction(unparsedOperator.substring(1), Array.class).consume(arguments);
            }
        ...
        return operator.hasEnoughParameters(arguments) ? operator.checkCondition(arguments, parser, getParentActor())
                : true;
    }
    ...
    private static enum Operator {
        ...
        LOWER_THAN("<", "lt", "lowerThan", "lesserThan") {
            ...
            public boolean checkCondition(final Array<String> conditionArguments, final LmlParser parser,
                    final Actor parent) throws LmlParsingException {
                final float firstArgument = Float.parseFloat(parseFirstArgument(conditionArguments, parser, parent));
                final float secondArgument = Float.parseFloat(parseSecondArgument(conditionArguments, parser, parent));
        ...
        IN(":", "in") {
            ...
            public boolean checkCondition(final Array<String> conditionArguments, final LmlParser parser,
                    final Actor parent) {
                final String firstArgument = parseFirstArgument(conditionArguments, parser, parent);
                final Array<String> secondArgument = AbstractLmlMacroParent
                        .toArgumentArray(parseSecondArgument(conditionArguments, parser, parent), parser, parent);
        ...
        MODULO("%", "mod", "md") {
            ...
            public boolean checkCondition(final Array<String> conditionArguments, final LmlParser parser,
                    final Actor parent) {
                final int firstArgument = Integer.parseInt(parseFirstArgument(conditionArguments, parser, parent));
                final int secondArgument = Integer.parseInt(parseSecondArgument(conditionArguments, parser, parent));
                ...
                if (conditionArguments.size > THIRD_ARGUMENT_INDEX) {
                    ...
                    conditionArguments.set(FIRST_OPERATOR_INDEX, conditionArguments.get(SECOND_OPERATOR_INDEX));
                    conditionArguments.set(SECOND_ARGUMENT_INDEX, conditionArguments.get(THIRD_ARGUMENT_INDEX));
                    return getOperator(conditionArguments.get(FIRST_OPERATOR_INDEX)).checkCondition(conditionArguments,
                            parser, parent);
                }
                return modulo == 0;
            }
        ...
        public abstract boolean checkCondition(final Array<String> conditionArguments, final LmlParser parser,
                final Actor parent);
        ...
        private static String parseFirstArgument(final Array<String> conditionArguments, final LmlParser parser,
                final Actor parent) {
            return parser.parseStringData(conditionArguments.get(FIRST_ARGUMENT_INDEX), parent);
        }

        private static String parseSecondArgument(final Array<String> conditionArguments, final LmlParser parser,
                final Actor parent) {
