...
public class EvaluateLmlMacroParser extends AbstractLmlMacroParser {
    ...
    protected CharSequence parseTextToAppend(final LmlParser parser, final LmlMacroData lmlMacroData) {
        ...
        if (arguments.size == 0) {
            throwErrorIfStrict(parser, "Evaluate macro needs at least one argument - method name. Received none.");
        } else {
            final Object methodResult = parser.findAction(getMethodName(arguments.first()), (Actor) null)
                    .consume(parser);
            if (arguments.size > 1) {
