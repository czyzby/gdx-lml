...
public abstract class AbstractConditionalLmlMacroParent extends AbstractLmlMacroParent {
    ...
    public void closeTag(final LmlParser parser) {
        ...
        final String[] textToAppend = textInTag.split(ELSE_OPENING + getTagNameIgnoringCaseRegex() + ELSE_CLOSING);

        if (arguments.size > 0 && checkCondition(parser)) {
    ...
    private boolean invokeConditionalAction(final String conditionArgument, final LmlParser parser) {
        final Object actionResult = parser.findAction(conditionArgument.substring(1), Array.class)
                .consume(getParentActor());
