...
public class LmlAttributes implements LmlSyntax {
    private static final ObjectMap<String, Value> STATIC_TABLE_VALUES = GdxMaps.newObjectMap("minHeight",
            Value.minHeight, "prefHeight", Value.prefHeight, "maxHeight", Value.maxHeight, "minWidth", Value.minWidth,
            "prefWidth", Value.prefWidth, "maxWidth", Value.maxWidth);

    public LmlAttributes() {
    }

    /** Registers the given value under passed name. These values should be generic and work only on context actors.
     * Where a Value is expected (for example - in tables' cell sizes), you can pass chosen valueName as the attribute
     * and the corresponding Value implementation will be taken from a map. */
    ...
    public static boolean parseBoolean(final Actor actor, final LmlParser parser, final String attributeValue) {
        ...
        throw new LmlParsingException(
                "Boolean attribute value expected (true or false, case ignored). Received: " + attribute + ".", parser);
    }

    /** @param attributeValue has to represent a value from Alignment utility enum. Can also be an action, bundle or
     *            preference, as long as after parsing it will be the name of Alignment constant.
    ...
    /** @param attributeValue expected action name. May (but does not have to) start with &amp;, which will be ignored
    ...
    public static ActorConsumer<Object, Object> parseAction(final Actor actor, final LmlParser parser,
        ...
        return attributeValue.charAt(0) == ACTION_OPERATOR ? parser.findAction(attributeValue.substring(1), actor)
                : parser.findAction(attributeValue, actor);
    }

    /** @param attributeValue action, number, %number, number% or registered value name.
     * @return if action: will invoke action - if the result is a Value instance, it will be returned. If number: a
     *         Fixed value will be returned. If %number: a value that returns a percent of actor height will be
     *         returned. If number%: a value that returns a percent of parent's height will be returned. If value name:
     *         Value will be extracted from the static values map and returned. */
    ...
    /** @param attributeValue action, number, %number, number% or registered value name.
     * @return if action: will invoke action - if the result is a Value instance, it will be returned. If number: a
     *         Fixed value will be returned. If %number: a value that returns a percent of actor width will be returned.
     *         If number%: a value that returns a percent of parent's width will be returned. If value name: Value will
     *         be extracted from the static values map and returned. */
    private static Value determineVerticalValue(final Table parent, final String attributeToParse,
        ...
        if (Strings.isFloat(attributeToParse)) {
            ...
            return Value.percentHeight(Float.parseFloat(attributeToParse.substring(0, attributeToParse.length() - 1)),
                    parent);
        ...
        if (!STATIC_TABLE_VALUES.containsKey(attributeToParse)) {
            throw new LmlParsingException("Invalid argument for table Value: " + attributeToParse
    ...
    private static Value determineHorizontalValue(final Table parent, final String attributeToParse,
        ...
        if (Strings.isFloat(attributeToParse)) {
            ...
            return Value.percentWidth(Float.parseFloat(attributeToParse.substring(0, attributeToParse.length() - 1)),
                    parent);
        } else if (attributeToParse.charAt(0) == PERCENT_OPERATOR) {
