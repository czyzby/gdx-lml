...
public class TextAreaLmlTagDataParser extends TextFieldLmlTagDataParser {
    ...
    @Override
    protected TextArea parseChildWithValidTag(final LmlTagData lmlTagData, final LmlParser parser) {
        return new TextArea(Strings.EMPTY_STRING, parser.getSkin(), getStyleName(lmlTagData, parser));
    }

    @Override
    protected LmlParent<TextField> parseParentWithValidTag(final LmlTagData lmlTagData, final LmlParser parser,
            final LmlParent<?> parent) {
        return new TextAreaLmlParent(lmlTagData, parseChild(lmlTagData, parser), parent, parser);
    }
}
