...
public class ProgressBarLmlTagDataParser extends AbstractLmlTagDataParser<ProgressBar> {
	...
	protected ProgressBar getNewInstanceWithoutStyle(final LmlParser parser, final float min,
	...
	protected ProgressBar getNewInstanceWithStyle(final LmlTagData lmlTagData, final LmlParser parser,
			final float min, final float max, final float stepSize, final boolean vertical) {
		return new ProgressBar(min, max, stepSize, vertical, parser.getSkin(), getStyleName(lmlTagData,
				parser));
	}

	protected float getMin(final LmlTagData lmlTagData, final LmlParser parser) {
		return lmlTagData.containsAttribute(MIN_ATTRIBUTE) ? LmlAttributes.parseFloat(null, parser,
				lmlTagData.getAttribute(MIN_ATTRIBUTE)) : DEFAULT_MIN;
	}

	protected float getMax(final LmlTagData lmlTagData, final LmlParser parser, final float min) {
		return lmlTagData.containsAttribute(MAX_ATTRIBUTE) ? LmlAttributes.parseFloat(null, parser,
				lmlTagData.getAttribute(MAX_ATTRIBUTE)) : min + DEFAULT_RANGE_SIZE;
	}
	...
	protected float getStepSize(final LmlTagData lmlTagData, final LmlParser parser, final float min,
		...
		if (lmlTagData.containsAttribute(STEP_SIZE_ATTRIBUTE)) {
			final String stepSizeAttribute =
					LmlAttributes.parseString(null, parser, lmlTagData.getAttribute(STEP_SIZE_ATTRIBUTE));
