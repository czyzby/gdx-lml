...
public class TableLmlTagDataParser extends AbstractLmlTagDataParser<Table> {
    ...
    private final ObjectMap<String, LmlTagAttributeParser> attributeParsers = new ObjectMap<String, LmlTagAttributeParser>(
            ATTRIBUTE_PARSERS);
    private final ObjectMap<String, LmlTagAttributeParser> cellAttributeParsers = new ObjectMap<String, LmlTagAttributeParser>(
            CELL_ATTRIBUTE_PARSERS);
    ...
    protected void parseAttributes(final LmlTagData lmlTagData, final LmlParser parser, final Actor actor) {
        ...
        for (final Entry<String, String> attribute : lmlTagData.getAttributes()) {
            ...
            if (cellAttributeParsers.containsKey(attribute.key)) {
                cellAttributeParsers.get(attribute.key).apply(((Table) actor).defaults(), parser, attribute.value,
                        lmlTagData);
            }
        }
    }
