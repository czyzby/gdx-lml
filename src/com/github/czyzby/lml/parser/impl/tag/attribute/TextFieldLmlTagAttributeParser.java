...
public enum TextFieldLmlTagAttributeParser implements LmlTagAttributeParser {
    ...
    PASSWORD_CHARACTER("passChar", "passwordCharacter") {
        ...
        protected void apply(final TextField textField, final LmlParser parser, final String attributeValue,
                final LmlTagData lmlTagData) {
            textField.setPasswordCharacter(LmlAttributes.parseString(textField, parser, attributeValue).charAt(0));
        }
    },
        ...
        protected void apply(final TextField textField, final LmlParser parser, final String attributeValue,
            ...
            if (lmlTagData.containsAttribute(SELECTION_END_ATTRIBUTE)) {
                textField.setSelection(selectionStart,
                        LmlAttributes.parseInt(textField, parser, lmlTagData.getAttribute(SELECTION_END_ATTRIBUTE)));
            } else {
        ...
        protected void apply(final TextField textField, final LmlParser parser, final String attributeValue,
            ...
            if (lmlTagData.containsAttribute(SELECTION_START_ATTRIBUTE)) {
                textField.setSelection(
                        LmlAttributes.parseInt(textField, parser, lmlTagData.getAttribute(SELECTION_START_ATTRIBUTE)),
                        selectionEnd);
            } else {
    ...
    protected abstract void apply(TextField textField, LmlParser parser, String attributeValue, LmlTagData lmlTagData);
