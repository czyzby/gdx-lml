...
public enum ScrollPaneLmlTagAttributeParser implements LmlTagAttributeParser {
	CLAMP("clamp") {
		@Override
		protected void apply(final ScrollPane scrollPane, final LmlParser parser,
	...
	FLICK_SCROLL_TAP_SQUARE_SIZE("flickScrollTapSquareSize", "tapSquareSize") {
		...
		protected void apply(final ScrollPane scrollPane, final LmlParser parser,
				final String attributeValue, final LmlTagData lmlTagData) {
			scrollPane.setFlickScrollTapSquareSize(LmlAttributes.parseFloat(scrollPane, parser,
					attributeValue));
		}
	},
	...
	OVERSCROLL("overscroll", "overscrollX", "overscrollY") {
		...
		private boolean determineOverscrollX(final ScrollPane scrollPane, final LmlParser parser,
			...
			if (lmlTagData.containsAttribute(OVERSCROLL_X_ATTRIBUTE)) {
				return LmlAttributes.parseBoolean(scrollPane, parser,
						lmlTagData.getAttribute(OVERSCROLL_X_ATTRIBUTE));
			} else if (lmlTagData.containsAttribute(OVERSCROLL_ATTRIBUTE)) {
				return LmlAttributes.parseBoolean(scrollPane, parser,
						lmlTagData.getAttribute(OVERSCROLL_ATTRIBUTE));
			}
		...
		private boolean determineOverscrollY(final ScrollPane scrollPane, final LmlParser parser,
			...
			if (lmlTagData.containsAttribute(OVERSCROLL_Y_ATTRIBUTE)) {
				return LmlAttributes.parseBoolean(scrollPane, parser,
						lmlTagData.getAttribute(OVERSCROLL_Y_ATTRIBUTE));
			} else if (lmlTagData.containsAttribute(OVERSCROLL_ATTRIBUTE)) {
				return LmlAttributes.parseBoolean(scrollPane, parser,
						lmlTagData.getAttribute(OVERSCROLL_ATTRIBUTE));
			}
	...
	OVERSCROLL_DISTANCE("overscrollDistance", "overscrollSpeedMin", "overscrollSpeedMax") {
		...
		private float getSpeedMin(final ScrollPane scrollPane, final LmlParser parser,
				final LmlTagData lmlTagData) {
		...
		private float getSpeedMax(final ScrollPane scrollPane, final LmlParser parser,
				final LmlTagData lmlTagData) {
	...
	SCROLLBAR_POSITION("scrollbarPositionX", "scrollbarPositionY") {
		...
		protected void apply(final ScrollPane scrollPane, final LmlParser parser,
				final String attributeValue, final LmlTagData lmlTagData) {
			final boolean positionX =
					lmlTagData.containsAttribute(SCROLLBAR_POSITION_X_ATTRIBUTE) ? LmlAttributes
							.parseBoolean(scrollPane, parser,
									lmlTagData.getAttribute(SCROLLBAR_POSITION_X_ATTRIBUTE))
							: DEFAULT_SCROLLBAR_POSITION;
			final boolean positionY =
					lmlTagData.containsAttribute(SCROLLBAR_POSITION_Y_ATTRIBUTE) ? LmlAttributes
							.parseBoolean(scrollPane, parser,
									lmlTagData.getAttribute(SCROLLBAR_POSITION_Y_ATTRIBUTE))
							: DEFAULT_SCROLLBAR_POSITION;
	...
	SCROLLING_DISABLED("scrollingDisabled", "scrollingDisabledX", "scrollingDisabledY") {
		...
		private boolean getDisableX(final ScrollPane scrollPane, final LmlParser parser,
				final LmlTagData lmlTagData) {
		...
		private boolean getDisableY(final ScrollPane scrollPane, final LmlParser parser,
				final LmlTagData lmlTagData) {
