...
public class ScrollPaneLmlTagDataParser extends AbstractLmlTagDataParser<ScrollPane> {
    ...
    static {
        ...
        for (final LmlTagAttributeParser parser : ScrollPaneLmlTagAttributeParser.values()) {
    ...
    protected ScrollPane parseChildWithValidTag(final LmlTagData lmlTagData, final LmlParser parser) {
        return new ScrollPane(null, parser.getSkin(), getStyleName(lmlTagData, parser));
    }

    @Override
    protected LmlParent<ScrollPane> parseParentWithValidTag(final LmlTagData lmlTagData, final LmlParser parser,
            final LmlParent<?> parent) {
        return new ScrollPaneLmlParent(lmlTagData, parseChild(lmlTagData, parser), parent, parser);
    }
}
