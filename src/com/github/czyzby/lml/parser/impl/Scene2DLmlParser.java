...
public class Scene2DLmlParser extends AbstractLmlParser {
    ...
    public static class ParserAlgorithm implements LmlSyntax {
        ...
        private void validateHierarchy() {
            if (!parser.widgetsHierarchy.isEmpty()) {
                throw new LmlParsingException(
                        "Unclosed parent tag: " + parser.widgetsHierarchy.getFirst().getTagName() + ".", parser);
            }
        }
        ...
        private boolean isParentRejectingCharacter(final char character) {
            return !parser.widgetsHierarchy.isEmpty() && !parser.widgetsHierarchy.getFirst().acceptCharacter(character);
        }
        ...
        private boolean isTagNameMatchingCurrentParent(final StringBuilder rawTagData) {
            return extractTagNameFromRawData(rawTagData)
                    .equalsIgnoreCase(parser.widgetsHierarchy.getFirst().getTagName());
        }
        ...
        private boolean isTagMatchingCurrentParentTag(final String tag) {
            return tag.equalsIgnoreCase(parser.widgetsHierarchy.getFirst().getTagName()) || tag.charAt(0) == MACRO_SIGN
        ...
        private void handleArgumentParsing(final Queue<Character> currentBuffer) {
            ...
            currentBuffer.poll();
            for (char argumentKeyChar = currentBuffer
                    .poll(); argumentKeyChar != ARGUMENT_CLOSING; argumentKeyChar = currentBuffer.poll()) {
        ...
        private boolean checkIfComment(final Queue<Character> currentBuffer) {
            ...
            if (currentBuffer.peek() != null && (currentBuffer.peek().charValue() == COMMENT_START_OPERATOR
                    || currentBuffer.peek().charValue() == SCHEMA_COMMENT_OPERATOR)) {
        ...
        private void handleMacro(final StringBuilder rawTagData) {
            ...
            if (macroData.isClosed()) {
                ...
                final LmlParent<?> macroParent = parser.macroParsers.get(macroData.getMacroName())
                        .parseMacroParent(parser, macroData, parser.getCurrentParent());
        ...
        private void handleParentTagOpening(final LmlTagData tagData) {
            ...
            final LmlParent<?> currentParent = parser.tagParsers.get(tagData.getTagName()).parseParent(tagData, parser,
                    oldParent);
            if (oldParent != null) {
