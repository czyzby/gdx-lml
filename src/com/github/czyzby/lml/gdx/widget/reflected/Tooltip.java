...
/** A simple component representing a tooltip. Can contain any actor, including complex component tables. Has to be
 * manually called to show and hide, although the logic is already done when using default Listener.
 *
 * @author MJ */
public class Tooltip extends Widget implements Disableable {
    ...
    public void show(final Actor parent, final float duration) {
        if (!disabled) {
            ...
            content.addAction(Actions.sequence(Actions.alpha(0f), Actions.fadeIn(duration, Interpolation.fade)));
            setInitialOffsets();
            setPosition();
        }
    }
    ...
    public void setPosition(final float x, final float y) {
        unmodifiedX = x;
        unmodifiedY = y;
        getStage().stageToScreenCoordinates(tempVector2.set(x + getPrefWidth() + X_OFFSET, y - getPrefHeight()));
    ...
    public void hide(final float duration) {
        clearActions();
        content.addAction(Actions.sequence(Actions.fadeOut(duration, Interpolation.fade), Actions.removeActor(),
    ...
    /** @param actor will have the tooltip attach with a specialized input listener that displays the tooltip after the
     *            cursor is over that actor for a specified amount of time. */
    ...
    private ClickListener prepareTooltipListener(final Actor toActor, final float showingDelay) {
        return new ClickListener() {
            ...
            public void enter(final InputEvent event, final float x, final float y, final int pointer,
                ...
                toActor.addAction(
                        Actions.sequence(Actions.delay(showingDelay, Actions.run(prepareTooltipShowingAction()))));
            }
            ...
            public boolean mouseMoved(final InputEvent event, final float x, final float y) {
                if (tooltip.isShown()) {
                    tooltip.getStage()
                            .screenToStageCoordinates(tooltip.tempVector2.set(Gdx.input.getX(), Gdx.input.getY()));
