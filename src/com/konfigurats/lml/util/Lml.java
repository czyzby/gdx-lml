package com.konfigurats.lml.util;
...
import com.konfigurats.lml.parser.impl.AbstractLmlParser;
import com.konfigurats.lml.parser.impl.Scene2DLmlParser;
import com.konfigurats.lml.parser.impl.dto.ActionContainer;
...
public class Lml implements LmlSyntax {
	...
	/** @param arguments will be converted to a LML-compatible array. Note that arrays, ranges and method
	 *            invocations can be stored together and are properly parsed by marcos that expect arrays. */
	public static String toArrayArgument(final Object... arguments) {
		if (arguments.length == 0) {
			...
			int index = 0;
			final StringBuilder argument =
					new StringBuilder(Nullables.toString(arguments[index++], NULL_ARGUMENT));
			for (; index < arguments.length; index++) {
				argument.append(ARGUMENT_SEPARATOR).append(
						Nullables.toString(arguments[index], NULL_ARGUMENT));
			}
	...
	public static class LmlParserBuilder {
		...
		/** @param argumentValue will be passed to the .lml file. Arguments can be referenced with
		 *            ${argumentKey}, case sensitive. */
		...
		/** @param arguments will be passed to the .lml file. Arguments can be referenced with ${argumentKey},
		 *            case sensitive. */
		...
		/** @param argumentValueBase will be added as range argument (ending with [0-rangeEnd]) with the given
		 *            key. */
		...
		/** @param argumentValueBase will be added as range argument (ending with [rangeStart-rangeEnd]) with
		 *            the given key. */
		public LmlParserBuilder rangeArgument(final String argumentKey, final String argumentValueBase,
		...
		/** @param actionExecutor executes a single action. Tag attributes that expect method names (like
		 *            onClick) will look for executors based on these keys. ActorConsumers take precedence
		 *            over methods added with reflection. */
		...
		/** @param actionExecutors execute a single action. Tag attributes that expect method names (like
		 *            onClick) will look for executors based on keys passed in this map. ActorConsumers take
		 *            precedence over methods added with reflection. Note that this method will add new
		 *            actions rather than replace the old ones (unless there are collisions in method keys). */
		public LmlParserBuilder actions(final ObjectMap<String, ActorConsumer<?, ?>> actionExecutors) {
			lmlParser.addActions(actionExecutors);
			return this;
		}

		/** @param actionContainer can contain multiple methods that will be extracted with reflection. As
		 *            opposed to ActorConsumers' keys, action container keys do not reference specific actions
		 *            - they point to specific containers. To access a method from action container, you can
		 *            use both methodName (although you cannot be sure which container will have its method
		 *            extracted) or containerKey.methodName. Note that ActionContainer implementation cannot
		 *            be anonymous and have to be GWT-reflected to work on HTML backend. */
		...
		/** @param actionContainers each can contain multiple methods that will be extracted with reflection. As
		 *            opposed to ActorConsumers' keys, action container keys do not reference specific actions
		 *            - they point to specific containers. To access a method from action container, you can
		 *            use both methodName (although you cannot be sure which container will have its method
		 *            extracted) or containerKey.methodName. Note that ActionContainer implementation cannot
		...
		/** @return LmlParser stored and modified by the builder. Note that due to complexity of registering
		 *         parser attributes, builder stores an instance of LmlParser the entire time, rather than
		 *         keeping the arguments and building the parser with this method. Use one instance of builder
		 *         for one parser only. */
