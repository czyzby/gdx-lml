 ...
 * <br> <br> Semi-immutable. On contrary to Array, ObjectMap is better encapsulated and ImmutableObjectMap is
 * an (almost) immutable collection. The only way to modify the collection is getting the iterator manually
 * and using its removing methods - original iterators' implementations operate on private fields (rather than
 * public getters/modifying methods) and could not have been rewritten. Still, standard usage of for-each
 * loops is safe. Deprecates and throws UnsupportedOperationExceptions on all operations that mutate the map.
 *
 * @author Nathan Sweet
 * @author MJ */
