package com.konfigurats.lml.util.gdx.collection.immutable;

import com.badlogic.gdx.utils.Array;
import com.badlogic.gdx.utils.ObjectSet;

/** An unordered set where the keys are objects. This implementation uses cuckoo hashing using 3 hashes, random
 * walking, and a small stash for problematic keys. Null keys are not allowed. No allocation is done except
 * when growing the table size. <br> <br> This set performs very fast contains and remove (typically O(1),
 * worst case O(log(n))). Add may be a bit slower, depending on hash collisions. Load factors greater than
 * 0.91 greatly increase the chances the set will have to rehash to the next higher POT size. <br> <br> *
 * Semi-immutable. On contrary to Array, ObjectSet is better encapsulated and ImmutableObjectSet is an
 * (almost) immutable collection. The only way to modify the collection is getting the iterator manually and
 * using its removing methods - original iterator implementation operates on private fields (rather than
 * public getters/modifying methods) and could not have been rewritten. Still, standard usage of for-each
 * loops is safe. Deprecates and throws UnsupportedOperationExceptions on all operations that mutate the set.
 *
 * @author Nathan Sweet
 * @author MJ */
