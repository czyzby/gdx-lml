...
// This is a temporary Method class fix for GWT platform. In LibGDX 1.9.2, Method#getReturnType()
// and Method#getEnclosingType() are broken and always return CachedTypeLookup.class, making it
// impossible for Autumn to work correctly with some methods. It will be removed eventually.
...
public class Method {
    ...
    public Method(final String name, final Class enclosingType, final Class returnType, final Parameter[] parameters,
            final boolean isAbstract, final boolean isFinal, final boolean isStatic, final boolean isDefaultAccess,
            final boolean isPrivate, final boolean isProtected, final boolean isPublic, final boolean isNative,
            final boolean isVarArgs, final boolean isMethod, final boolean isConstructor, final int methodId,
            final Annotation[] annotations) {
        this.name = name;
    ...
    /** Invokes the method on the given object. Ignores the object if this is a static method. Throws an
     * IllegalArgumentException if the parameters do not match.
     *
    ...
    boolean match(final Class... types) {
        if (types == null) {
        ...
        for (int i = 0; i < types.length; i++) {
            final Type t1 = parameters[i].getType();
            final Type t2 = ReflectionCache.getType(types[i]);
