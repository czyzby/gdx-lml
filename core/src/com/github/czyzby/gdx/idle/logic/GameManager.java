...
public class GameManager {
	...
	@Inject
	private DamageLabelPool damageLabelPool;
	@Inject
	...
	@Asset("game/locations.atlas")
	private TextureAtlas locationsAtlas;
	@Asset("game/monsters.atlas")
	private TextureAtlas monstersAtlas;
	@Asset(value = { "sfx/sounds/punch0.ogg", "sfx/sounds/punch1.ogg", "sfx/sounds/punch2.ogg",
			"sfx/sounds/punch3.ogg", "sfx/sounds/punch4.ogg" }, type = Sound.class)
	private Array<Sound> hitSounds;
	@Asset(value = { "sfx/sounds/heal0.ogg", "sfx/sounds/heal1.ogg", "sfx/sounds/heal2.ogg" }, type = Sound.class)
	private Array<Sound> healSounds;
	...
	private void updatePlayerHealth() {
		gameView.playerHealthBar.setValue((float) currentPlayerHealthAmount
				/ (float) getTotalPlayerHealthAmount());
	...
	private void updateMonsterHealth() {
		gameView.monsterHealthBar
				.setValue((float) currentMonsterHealthAmount / (float) currentMonster.health);
	...
	private void dealPlayerDamage(final int damage, final boolean heal) {
		if (currentPlayerHealthAmount <= 0 || currentPlayerHealthAmount == getTotalPlayerHealthAmount()
				&& heal) {
			return;
		}
		if (heal) {
		...
		currentPlayerHealthAmount =
				Math.min(getTotalPlayerHealthAmount(), Math.max(currentPlayerHealthAmount, 0));
		updatePlayerHealth();
		if (currentPlayerHealthAmount <= 0) {
