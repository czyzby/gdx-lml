...
public class DamageLabelPool {
    ...
    private final LinkedList<Label> queue = new LinkedList<Label>();
    private final ObjectMap<Label, Runnable> returningActions = GdxMaps.newObjectMap();

    @Initiate
    private void buildPrototypes(final InterfaceService interfaceService) {
        final LmlParser parser = interfaceService.getParser();
        parser.parseTemplate(Gdx.files.internal("templates/prototypes/labels.lml"));
        final ObjectMap<String, Actor> labels = parser.getActorsMappedByIds();
